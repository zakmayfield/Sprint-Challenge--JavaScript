...
class DinoRex extends Jurassic{
...
class DinoSteg extends Jurassic{
  constructor(dinoAttrs){
    super(dinoAttrs)
  }
}

class DinoVelo extends Jurassic{
  constructor(dinoAttrs){
    super(dinoAttrs)
  }
}

const tRex = new DinoRex({
  name: 'tyrannosaurus',
  diet: 'carnivorous',
  weight: '7000kg',
  length: '12m',
...
.map((graduate) => {
  return {university: graduate.university}
})

.filter((object) => {
  let uniName = object.university;
  return uniName.toLowerCase().includes('uni')
})
...
console.log(`The total population of all of these animals: ${populationTotal}`);
